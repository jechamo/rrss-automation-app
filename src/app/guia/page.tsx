...
const SECTIONS = [
  ...
  ["mapa", "Mapa de la app", "Árbol funcional de hasta tres niveles generado desde rutas y navegación real del código. Reconoce a la vez navbar, menú inferior, sidebar, drawer, pestañas y tarjetas; puedes filtrarlo, copiarlo y verificar sus rutas con Playwright."],
...
const FLOWS: Flow[] = [
  {
    ...
    steps: ["Entra en Contenido y pulsa Generar desde viral.", "Selecciona un viral: se usa su patrón, no se copia su contenido.", "Elige fal.ai y el modelo. Auto prioriza coste; en manual puedes elegir Seedance 1/2, Kling 2.5/3, Veo 3.1 o Luma.", "Compara el coste por segundo que aparece junto a cada modelo.", "Elige 5/10/15 segundos por corte y decide Automático o una cantidad manual de 1 a 6.", "Elige voz de ElevenLabs y decide si Gemini debe analizar el vídeo fuente.", "Revisa el Plan de montaje: objetivo, apoyo visual, cortes, segundos solicitados y coste.", "Pulsa Preparar guion y prompts. Este paso usa la IA de guion, pero todavía no envía nada a fal.ai.", "Lee el hook y edita cada prompt si quieres cambiar sujeto, acción, cámara, luz o estilo.", "Pulsa Aprobar prompts y generar. La pipeline reutiliza exactamente lo aprobado y fal.ai respeta esa cantidad máxima.", "Observa Extraer → Guion → Vídeo → Locución → Montaje.", "Abre la pieza, reproduce final.mp4 y revisa texto, cortes y CTA."],
    output: "Clips fal.ai + audio ElevenLabs + montaje vertical + subtítulos quemados abajo + cierre animado de 3 s si el proyecto tiene logo.",
  ...
  },
  {
    ...
    output: "Presentador HeyGen original + copia final vertical con subtítulos obligatorios + cierre animado de 3 s si el proyecto tiene logo.",
  ...
  },
  {
    ...
    steps: ["Pulsa + Contenido propio.", "Escribe la función y pulsa Analizar. La app contrasta código, mapa y navegador real.", "Playwright prueba automáticamente el recorrido: puede descubrir tabs o filtros intermedios y corregir un selector cuyo tag o texto no coincide.", "Revisa URL, pasos humanos y el JSON ya validado. Si lo editas, pulsa Probar pasos otra vez antes de generar.", "Selecciona Automática y configura login si hace falta.", "Elige fal.ai o HeyGen.", "Con fal.ai, pulsa Preparar guion y prompts, edita los cortes B-roll y después Aprobar prompts y generar.", "Con HeyGen no hay prompts fal.ai: aprueba el plan directamente.", "Playwright emula iPhone, navega y guarda la grabación y nav_log.", "El montaje intercala hook, demo y cierre; luego quema subtítulos.", "Si solo falla o cambia la navegación, pulsa Regrabar navegación: se valida, se graba y se remonta conservando vídeo, voz y guion."],
    output: "Screencast Playwright + B-roll fal o presentador HeyGen + voz + subtítulos + cierre animado de marca si hay logo.",
    errors: ["Selector roto: la app intenta repararlo con elementos visibles y controles intermedios seguros; si no puede, muestra el paso exacto y conserva la opción manual.", "Chromium ausente: usa subida/REC manual y Remontar sin créditos.", "Login fallido: actualiza las credenciales del proyecto; la contraseña nunca vuelve en la API."],
  },
  {
    ...
    steps: ["Crea Contenido propio y elige Subir después.", "La app genera guion y recursos aunque aún no exista screencast.", "Abre la tarjeta de la pieza y pulsa Subir vídeo de la app.", "Selecciona el fichero y espera la confirmación.", "Pulsa Remontar sin créditos. Se reutilizan clips, presentador, locución, guion y subtítulos; no se consulta ningún proveedor.", "Comprueba la nueva versión final. El montaje anterior no se sobrescribe si el proceso falla."],
  ...
  },
  {
    ...
    steps: ["Abre Estudio multimedia o pulsa REC en una pieza propia.", "Pulsa Abrir app en formato móvil.", "Inicia sesión directamente en la ventana móvil.", "Vuelve a RRSS Studio y pulsa REC.", "En el diálogo del navegador elige la ventana móvil. La app no puede elegirla por ti.", "Navega mostrando solo los pasos que quieres conservar.", "Vuelve al grabador y pulsa STOP, o detén la compartición desde el navegador.", "Reproduce el preview, pon nombre y pulsa Guardar grabación. En una ventana baja, desplaza el contenido central: las acciones permanecen visibles.", "Si empezaste desde una pieza, queda adjuntada; si empezaste desde Estudio, queda libre en la mediateca.", "En una pieza existente: Regrabar navegación repite Playwright; Reemplazar vídeo adjunta un archivo; Quitar solo lo desvincula; Remontar sin créditos reutiliza todos los recursos existentes."],
  ...
  {
    id: "mix", title: "Mediateca → MIX", summary: "Edita en una timeline profesional, guarda borradores y genera versiones sin reemplazar montajes anteriores.",
    needs: ["Al menos un vídeo en la mediateca", "Texto exacto para subtítulos", "FFmpeg/ffprobe detectados"],
    steps: [
      "Abre Estudio multimedia. La biblioteca fija de la izquierda separa Pistas app y Capas; Gestionar mediateca abre el diálogo completo solo cuando necesitas subir, renombrar o eliminar archivos.",
      "Si tienes varios proyectos, usa el selector superior. Antes de salir se confirma el cambio y la timeline actual se guarda como borrador.",
      "Pon nombre al borrador y, si corresponde, elige una pieza destino. Los cambios se guardan automáticamente y la etiqueta superior confirma Borrador guardado.",
      "Arrastra una grabación o vídeo a Principal. Puedes insertar más bloques en esa misma calle; los momentos protegidos mantienen su candado.",
      "Arrastra clips, presentadores o vídeos subidos a Nueva capa. Cada recurso crea una calle independiente; no hay un máximo fijo y la timeline usa scroll interno cuando crece.",
      "También puedes usar + Principal y + Capa si prefieres no arrastrar.",
      "Usa Preparar secuencial para convertir la selección actual en una sucesión de bloques principales. Usa Preparar en capas para conservar la navegación —o el primer bloque— como Principal y colocar los apoyos encima. Repetir el plan no importa todos los recursos ni crea duplicados.",
      "Pulsa Play/Pausa para reproducir el montaje vivo, Stop para volver al inicio y mueve el cabezal o el zoom para trabajar con precisión. No existe un segundo botón de vista previa: el visor central siempre responde a la timeline.",
      "Haz clic en un bloque para editarlo en Propiedades. Arrastra el cuerpo de una capa para moverla y sus asas laterales para recortar o ampliar; también puedes ajustar Empieza, Entrada y Salida con números.",
      "En una capa elige pantalla completa o PIP. Para PIP define posición y tamaño; una cobertura sobre un momento protegido muestra un aviso.",
      "Elige una locución. Si está vinculada a un guion, Subtítulos obligatorios se rellena automáticamente; un audio libre conserva el texto que hayas escrito.",
      "Añade música y regula su volumen. Las capas visuales no mezclan su audio, evitando voces duplicadas.",
      "Activa o desactiva Cierre de marca. Solo está disponible cuando el proyecto tiene logo y se representa como el último bloque de la timeline.",
      "Abre Versiones para recuperar otro borrador, crear uno nuevo o consultar resultados renderizados. Los vídeos finales/MIX no vuelven a la biblioteca como material de edición.",
      "Pulsa Generar versión cuando vídeo, duración y subtítulos sean válidos. Ver resultado reproduce el fichero exacto; Usar como final requiere una confirmación y no borra el borrador.",
    ],
    output: "Borrador recuperable y nueva versión MIX consultable, con subtítulos inferiores siempre visibles y cierre animado opcional si hay logo.",
    errors: ["Un asset eliminado/referenciado bloquea el render con explicación.", "Si no hay margen para armonizar, desbloquea/recorta conscientemente un bloque o añade otro recurso.", "Una grabación manual no tiene pasos automáticos: protege y recorta sus momentos desde la timeline.", "Sin libass no se crea un final sin subtítulos.", "Música demasiado alta: baja el deslizador; el original no se modifica."],
  },
  {
...
export default function GuiaPage() {
  return <div className="mx-auto max-w-7xl pb-16">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[250px_1fr]">
      <aside className="glass h-fit p-4 lg:sticky lg:top-4"><div className="text-xs font-semibold uppercase tracking-wider text-white/40">Índice</div><nav className="mt-3 flex flex-col gap-1 text-sm"><a href="#inicio" className="rounded-lg px-2 py-1.5 text-white/65 hover:bg-white/5">Primeros 10 minutos</a><a href="#secciones" className="rounded-lg px-2 py-1.5 text-white/65 hover:bg-white/5">Todas las secciones</a><a href="#mapa-app" className="rounded-lg px-2 py-1.5 text-white/65 hover:bg-white/5">Mapa de la app</a><a href="#flujos" className="rounded-lg px-2 py-1.5 text-white/65 hover:bg-white/5">Flujos de vídeo</a>{FLOWS.map((flow) => <a key={flow.id} href={`#${flow.id}`} className="rounded-lg px-2 py-1.5 text-xs text-white/45 hover:bg-white/5">{flow.title}</a>)}<a href="#reglas" className="rounded-lg px-2 py-1.5 text-white/65 hover:bg-white/5">Reglas y seguridad</a></nav></aside>

      <main className="space-y-7">
        <section id="inicio" className="glass scroll-mt-4 p-6"><h2 className="text-2xl font-bold">Primeros 10 minutos</h2><ol className="mt-4 grid gap-3 md:grid-cols-2">{["Ve a Ajustes y prueba Claude, proveedores y herramientas del sistema.", "Crea un proyecto con URL y, si puedes, código local/GitHub.", "Espera el dossier y el mapa de la app; corrige el dossier y verifica las rutas privadas si lo necesitas.", "Genera competencia, leads y virales; usa Buscar más sin perder lo manual.", "Elige Contenido desde viral o Contenido propio para mostrar tu app.", "Revisa cada pieza; usa Estudio para REC/STOP, reutilización y MIX.", "Publica solo tras reproducir el final y revisar subtítulos abajo."].map((step, index) => <li key={step} className="rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-white/65"><span className="mr-2 text-lg font-bold text-[var(--color-accent-2)]">{index + 1}</span>{step}</li>)}</ol></section>
        ...
        <section id="mapa-app" className="glass scroll-mt-4 p-6"><h2 className="text-2xl font-bold">Cómo usar el mapa de la app</h2><ol className="mt-4 space-y-2">{["Indica la URL y la fuente de código al crear el proyecto. En Contexto escribe solo reglas útiles, por ejemplo: analizar cliente normal y excluir administración; no necesitas pegar un mapa manual.", "La pipeline inspecciona rutas, enlaces, navigate/router, layouts, navbars, barras inferiores, sidebars, drawers, pestañas y tarjetas. Si conviven varios sistemas, conserva varias raíces en el mismo árbol.", "Abre Mapa de la app. Cada entrada muestra superficie, ruta, login, rol, condiciones y evidencia fichero:línea. El árbol nunca supera tres niveles.", "Usa los filtros para revisar una superficie o los estados pendientes/condicionales. Las rutas con :id no reciben identificadores inventados.", "Guarda antes las credenciales cifradas del proyecto si las rutas son privadas y pulsa Verificar rutas. Playwright inicia sesión y visita únicamente rutas conocidas del mismo origen; no pulsa Guardar, Eliminar, Comprar ni otras acciones de negocio.", "Copia el Markdown si necesitas compartirlo. La app reutiliza internamente el mapa para enriquecer dossier, funcionalidades demostrables y futuros pasos Playwright."].map((step, index) => <li key={step} className="flex gap-3 rounded-xl border border-white/8 bg-black/15 p-3 text-sm text-white/65"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--color-accent)]/20 text-xs font-bold text-[var(--color-accent-2)]">{index + 1}</span><span>{step}</span></li>)}</ol><p className="mt-4 text-xs leading-relaxed text-white/40">Si no aportas código, se crea un mapa inicial a partir de las páginas rastreadas y queda marcado como pendiente. Para una SPA privada o rutas dinámicas, el código y el login mejoran mucho la precisión.</p></section>
